  ...
  return (
    <div style={{ minHeight: "100vh", background: D.bg, color: D.text, fontFamily: "'Inter',-apple-system,sans-serif", padding: "20px 14px 48px", boxSizing: "border-box", WebkitTextSizeAdjust: "100%", transition: "background 0.3s", position: "relative" }}>
      ...
      <div style={{ maxWidth: 500, margin: "0 auto" }}>
        ...
        {tab === "goals" && <>
          ...
          {!goalStarted ? <>
            ...
            <div style={{ background: D.summaryBg, border: `1px solid ${D.summaryBorder}`, borderRadius: 14, padding: "18px", marginBottom: 12 }}>
              ...
              {/* Progress bar */}
              <div style={{ height: 12, background: D.input, borderRadius: 99, overflow: "hidden", marginBottom: 6 }}>
                <div style={{ height: "100%", width: `${progressPct}%`, background: goalReached ? "#10b981" : "linear-gradient(90deg,#6366f1,#10b981)", borderRadius: 99, transition: "width 0.6s ease" }} />
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 16 }}>
                <span style={{ color: "#10b981", fontWeight: 600 }}>{fmt(savedTotal)} saved</span>
                <span style={{ color: D.sub }}>{progressPct.toFixed(1)}%</span>
              </div>

              {/* Stats */}
              <div style={{ display: "flex", justifyContent: "space-around", marginBottom: 18 }}>
                <div style={{ textAlign: "center" }}>
                  <p style={{ color: D.sub, fontSize: 10, letterSpacing: 1, textTransform: "uppercase", margin: "0 0 4px" }}>Streak</p>
                  <p style={{ color: "#f59e0b", fontWeight: 800, fontSize: 24, margin: 0 }}>{streak} 🔥</p>
                </div>
                <div style={{ textAlign: "center" }}>
                  <p style={{ color: D.sub, fontSize: 10, letterSpacing: 1, textTransform: "uppercase", margin: "0 0 4px" }}>Daily</p>
                  <p style={{ color: "#10b981", fontWeight: 800, fontSize: 24, margin: 0 }}>{fmt(dailySaving)}</p>
                </div>
                <div style={{ textAlign: "center" }}>
                  <p style={{ color: D.sub, fontSize: 10, letterSpacing: 1, textTransform: "uppercase", margin: "0 0 4px" }}>Days Left</p>
                  <p style={{ color: D.text, fontWeight: 800, fontSize: 24, margin: 0 }}>{goalReached ? "✓" : daysLeft}</p>
                </div>
              </div>
              ...
              {goalReached ? (
                <div style={{ textAlign: "center" }}>
                  <p style={{ color: "#10b981", fontWeight: 800, fontSize: 20, margin: "0 0 14px" }}>🎉 Goal Reached!</p>
                  <button onClick={resetGoal} style={{ width: "100%", background: D.input, border: `1px solid ${D.border}`, borderRadius: 12, color: D.sub, padding: "13px", fontSize: 14, cursor: "pointer", fontWeight: 600 }}>
                    Start New Goal
                  </button>
                </div>
              ) : (
                <>
                  <button onClick={handleCheckin} disabled={checkedInToday} style={{
                    width: "100%", border: "none", borderRadius: 12, padding: "15px", fontSize: 15, fontWeight: 700,
                    cursor: checkedInToday ? "default" : "pointer", transition: "all 0.2s",
                    background: checkedInToday ? D.input : "linear-gradient(135deg,#6366f1,#10b981)",
                    color: checkedInToday ? D.sub : "#fff",
                    opacity: checkedInToday ? 0.7 : 1,
                    marginBottom: 10,
                  }}>
                    {checkedInToday ? "✓ Checked in today" : "I stuck to my budget today ✓"}
                  </button>
                  {streak > 0 && !checkedInToday && (
                    <p style={{ textAlign: "center", color: D.sub, fontSize: 11, margin: 0 }}>Don't break your {streak} day streak! 🔥</p>
                  )}
                  <button onClick={resetGoal} style={{ width: "100%", background: "transparent", border: "none", color: D.muted, padding: "10px", fontSize: 11, cursor: "pointer", marginTop: 6 }}>
                    Reset goal
                  </button>
                </>
              )}
        ...
        <p style={{ textAlign: "center", color: D.muted, fontSize: 10, marginTop: 20, opacity: 0.3 }}>
